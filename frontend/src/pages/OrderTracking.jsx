...
export default function OrderTracking() {
  ...
  const handleTrackOrder = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.message || "Invalid Order ID or Order not found.");
      }
  ...
  return (
    <div className="order-tracking-container">
      <style>{`
        ...
        /* Grid Layout for Details and Timeline */
        .ot-grid {
          display: grid;
      ...
      <div className="ot-header-section">
        ...
        <p className="ot-subtitle">Enter your Order ID below to check the current status of your order</p>
      ...
      {loading && (
        <div className="ot-loading">
          ...
          <h3 style={{ color: '#2e7d32' }}>Fetching Order Details...</h3>
          <p style={{ color: '#666' }}>Please wait while we securely retrieve your order status.</p>
        </div>
      )}
      ...
      {trackingData && !loading && (
        <div className="ot-results">
          ...
          <div className="ot-stepper-card">
            ...
            {/* Calculate fill width based on completed steps */}
            <div 
            ...
            {trackingData.timeline.map((step, index) => {
              ...
              return (
                <div key={step.status} className={`ot-step ${step.completed ? 'completed' : ''}`}>
                  ...
                  <div className="ot-step-date">{step.completed ? formatDate(step.date) : `Expected on ${formatDate(step.date)}`}</div>
                </div>
              );
            })}
          </div>
          ...
          <div className="ot-grid">
            ...
            <div className="ot-card">
              ...
              <div className="ot-product-info">
                <div className="ot-product-img">
                  {/* Using a placeholder since we don't have images in the backend query output directly, but it matches the design */}
                ...
                <div className="ot-product-details">
                  ...
                  <div className="ot-seller">Seller: AgroMitra Verified Store</div>
                  {trackingData.orderItems && trackingData.orderItems.length > 1 && (
                    <div style={{ fontSize: 13, color: '#4caf50', fontWeight: 600 }}>+ {trackingData.orderItems.length - 1} more items</div>
                  )}
                </div>
              </div>
            ...
            {/* Order Timeline Column */}
            <div className="ot-card">
              <div className="ot-card-title">Order Timeline</div>
                  ...
                  <div key={idx} className={`ot-timeline-item ${step.completed ? 'completed' : ''}`}>
                    ...
                    <div className="ot-timeline-content">
                      <div className="ot-timeline-text">
                        <h5>{step.label}</h5>
                        <p>{step.description || 'Pending'}</p>
                      </div>
                      <div className="ot-timeline-time">
                        <div>{formatDate(step.date)}</div>
                        <div>{step.completed ? formatTime(step.date) : 'Expected'}</div>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          <div className="ot-greeting">
            ...
            Thank you for choosing AgroMitra. Happy Farming! 🌿
          </div>
        </div>
      )}
    </div>
